...
export async function deleteEnterprise(enterpriseId: string) {
	...
	if (!enterprise) throw new Error("La empresa no existe.");

	if (
		!canDeleteRecordByCreator(
			user,
			(enterprise.created_by as string | null) ?? null,
		)
	) {
		throw new Error(
			"No puedes eliminar esta empresa porque no fue creada por ti.",
		);
	}
